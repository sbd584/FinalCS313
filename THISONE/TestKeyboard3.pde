...
int textWidth;
int timer;
boolean rotation1,rotation2,recording,dot,playing,stopped,muted;
...
void setup() {
  ...
  note1 = new Music_note(150,20,50,0,0);
  note2 = new Music_note(150,20,1200,0,0);
}
...
void draw() {
  if(GUI_change == 1){
    ...
    note1.display(music_timer);
    note2.display(music_timer);
...
void mouseClicked() {
  // Record
  if(dist (315,120,mouseX,mouseY) <= 0){
    if (record_dot < 255 ) {
      record = 0;
      record_dot = 255;
      // 50
